...
class ScreenQuad
{
    ...
    void PassShaderUniforms(Camera* camera, vector<PointLight*> &pointLights)
    {
        screenQuadShader->setVec3f(camera->getPosition(), "cameraPos");

        // Pass vector of point lights to shader
       // glUniform1i(glGetUniformLocation(screenQuadShader.ID, "NR_LIGHTS"), pointLights.size());
